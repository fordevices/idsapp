 ...
 * 
 * ES Module for CAN data management - Normal CAN Data in the database
 ...
 * the only function that has any hardcoded table / col references is inferCANFieldType
...
function inferCANFieldType(fieldName) {
    ...
    if (name === 'label') {
        ...
        return 'hex';
    } else if (name.includes('candata') || name.includes('data')) {
        return 'hex';
    } else if (name.includes('timestamp') || name.includes('time')) {
        return 'datetime';
    } else if (name.includes('priority') || name.includes('severity')) {
        return 'select';
    } else if (name.includes('datalength') || name.includes('length')) {
        return 'number';
...
/**
 * Load database information and display it
 */
    ...
    try {
        ...
        if (response.ok) {
            ...
            if (databaseNameElement) {
                databaseNameElement.textContent = dbInfo.name;
            }
        } else {
            const databaseNameElement = document.getElementById('databaseName');
            if (databaseNameElement) {
                databaseNameElement.textContent = 'Connection error';
            }
        }
    } catch (error) {
...
async function initializeApp() {
    try {
        ...
        console.log('CAN data table metadata loaded:', currentTableMeta);
    } catch (error) {
        handleError('Failed to initialize CAN data management', error);
    }
}

/**
 * Display paginated CAN data records with label filtering
...
async function displayallitems(resetPagination = false) {
    try {
        ...
        console.log('Paginated CAN Items JSON from DB:', result.data?.length || 0, 'items');
        ...
        if (!Array.isArray(result.data) || result.data.length === 0) {
            if (currentOffset === 0) {
                console.log('No CAN data returned from database');
                currentRows = [];
                const tableContainer = document.getElementById('candata-list-div');
                if (tableContainer) {
                    tableContainer.innerHTML = '<p>No CAN data found.</p>';
                }
        ...
        handleError('Failed to display CAN data', error);
    }
}

/**
 * Display filtered CAN data records
...
async function displayfiltereditems(searchTerm) {
    try {
        ...
        handleError('Failed to display filtered CAN data', error);
    }
}
...
async function prepareAddNewForm() {
    try {
        ...
        // Show the modal
        const modal = new bootstrap.Modal(document.getElementById('addnewcanform'));
        modal.show();
...
/**
 * Prepare the edit form with existing CAN data
...
async function prepareEditForm(index) {
    try {
        ...
        if (!rowData) {
            throw new Error('CAN data record not found');
        }
...
async function deleterow(index) {
    try {
        ...
        if (confirm('Are you sure you want to delete this CAN data record?')) {
            await deleteRow(currentTable, rowKey);
            await displayallitems(true); // Refresh the table with reset pagination
        }
    } catch (error) {
        handleError('Failed to delete CAN data record', error);
    }
}

/**
 * Handle form submission for adding/updating CAN data
 */
...
/**
 * Save CAN data (create or update)
...
async function saveRow(formData) {
    try {
        ...
        const modal = bootstrap.Modal.getInstance(document.getElementById('addnewcanform'));
        if (modal) {
            modal.hide();
        }
        ...
        handleError('Failed to save CAN data', error);
    }
}
...
document.addEventListener('DOMContentLoaded', async () => {
    try {
        ...
        handleError('Failed to initialize CAN data management application', error);
    }
});
