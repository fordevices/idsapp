        ...
        for (const columnName of formColumns) {
            ...
            if (fieldType === 'unit') {
                ...
                switch (fieldType) {
                    ...
                    case 'time':
                        inputType = 'time';
                        // Force updatedtime to current time when editing, otherwise set default to current time
                        if (columnName === 'updatedtime' && rowData) {
                            const now = new Date();
                    ...
                    case 'decimal':
                        inputType = 'number';
                        // Ensure the value is properly converted to string for the input field
                        if (rowData && rowData[columnName] !== null && rowData[columnName] !== undefined) {
                            inputValue = String(rowData[columnName]);
                        } else {
                            inputValue = '';
                        }
                        break;
 ...
 * @param {Function} refreshCallback - Callback function for refresh action
 * @param {string} [containerId] - Optional container ID, will try to find one automatically if not provided
 * @throws {Error} If table display fails
 */
function displaytable(rowsData, prepareEditCallback, deleterowCallback, refreshCallback, containerId) {
    try {
        ...
        if (containerId) {
            ...
            const commonIds = ['candata-list-div', 'attackdata-list-div'];
            for (const id of commonIds) {
                tableContainer = document.getElementById(id);
        ...
        rowsData.forEach((row, index) => {
            ...
            headers.forEach(header => {
                const dataCell = createElement('span', { 
                    className: 'cell',
                    textContent: row[header] !== null && row[header] !== undefined ? row[header] : ''
                });
        ...
        // Try to find any available table container for error display
        const commonIds = ['candata-list-div', 'attackdata-list-div'];
        let tableContainer;
        for (const id of commonIds) {
            tableContainer = document.getElementById(id);
...
function formatColumnNameForDisplay(columnName) {
    try {
        ...
        // Handle CAN data specific patterns
        formatted = formatted.replace(/(signal)(\d+)/gi, '$1 $2');
        formatted = formatted.replace(/(of)(id)/gi, '$1 ID');
        formatted = formatted.replace(/(security)(ans)/gi, '$1 Answer');
        formatted = formatted.replace(/(created)(on|time)/gi, '$1 $2');
        formatted = formatted.replace(/(updated)(on|time)/gi, '$1 $2');
        formatted = formatted.replace(/(user)(name)/gi, '$1 $2');
